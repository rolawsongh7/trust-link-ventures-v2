-- Now run the enhanced CRM schema with all additional features
...
-- Create storage policies for quotes bucket
CREATE POLICY "Users can view their quote files" ON storage.objects
FOR SELECT USING (bucket_id = 'quotes' AND auth.uid() IS NOT NULL);

CREATE POLICY "Users can upload quote files" ON storage.objects
FOR INSERT WITH CHECK (bucket_id = 'quotes' AND auth.uid() IS NOT NULL);

CREATE POLICY "Users can update quote files" ON storage.objects
FOR UPDATE USING (bucket_id = 'quotes' AND auth.uid() IS NOT NULL);

CREATE POLICY "Users can delete quote files" ON storage.objects
FOR DELETE USING (bucket_id = 'quotes' AND auth.uid() IS NOT NULL);

-- Create storage policies for supplier logos bucket
...
FOR INSERT WITH CHECK (bucket_id = 'supplier-logos' AND auth.uid() IS NOT NULL);

CREATE POLICY "Users can update supplier logos" ON storage.objects
FOR UPDATE USING (bucket_id = 'supplier-logos' AND auth.uid() IS NOT NULL);

CREATE POLICY "Admins can delete supplier logos" ON storage.objects
FOR DELETE USING (bucket_id = 'supplier-logos' AND get_user_role(auth.uid()) = 'admin');