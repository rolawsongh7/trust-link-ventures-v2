...
-- Update user_roles table to use the enum and ensure proper structure
...
-- Enhanced RLS Policies for Leads
DROP POLICY IF EXISTS "Admins can manage all leads" ON public.leads;
DROP POLICY IF EXISTS "Sales reps can view assigned leads" ON public.leads;
...
CREATE POLICY "Sales reps can view assigned leads" ON public.leads
FOR SELECT TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) = 'sales_rep'::user_role 
  AND (assigned_to = auth.uid() OR assigned_to IS NULL)
...
CREATE POLICY "Sales reps can manage assigned leads" ON public.leads
FOR ALL TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) = 'sales_rep'::user_role 
  AND (assigned_to = auth.uid() OR assigned_to IS NULL)
...
-- Enhanced RLS Policies for Quotes
DROP POLICY IF EXISTS "Admins can manage all quotes" ON public.quotes;
DROP POLICY IF EXISTS "Sales reps can view assigned quotes" ON public.quotes;
...
CREATE POLICY "Sales reps can manage their quotes" ON public.quotes
FOR ALL TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) = 'sales_rep'::user_role 
...
-- Enhanced RLS Policies for Orders
DROP POLICY IF EXISTS "Admins can manage all orders" ON public.orders;
DROP POLICY IF EXISTS "Sales reps can view assigned orders" ON public.orders;
...
CREATE POLICY "Sales reps can manage their orders" ON public.orders
FOR ALL TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) = 'sales_rep'::user_role 
  AND (created_by = auth.uid() OR created_by IS NULL)
...
-- Enhanced RLS Policies for Communications
CREATE POLICY "Sales reps can manage communications for assigned customers" ON public.communications
FOR ALL TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) IN ('admin'::user_role, 'sales_rep'::user_role)
  AND (
    public.get_user_role_secure(auth.uid()) = 'admin'::user_role
    OR created_by = auth.uid()
    OR customer_id IN (
      SELECT id FROM public.customers 
      WHERE assigned_to = auth.uid() OR public.get_user_role_secure(auth.uid()) = 'admin'::user_role
    )
  )
)
WITH CHECK (
...
-- Enhanced RLS Policies for Activities
CREATE POLICY "Sales reps can manage activities for assigned entities" ON public.activities
FOR ALL TO authenticated
USING (
  public.get_user_role_secure(auth.uid()) IN ('admin'::user_role, 'sales_rep'::user_role)
  AND (
