...
const StatsCard = ({
  ...
  return <div className="animate-fade-in-right">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8 animate-fade-in-left">
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex items-center gap-2">
                  {heroStories.map((_, index) => <div key={index} className={`h-1 w-8 rounded-full transition-all duration-500 ${index === currentSlide ? 'bg-primary' : 'bg-primary/30'}`} />)}
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <div className="p-1 bg-primary/10 rounded-full">
                    {React.createElement(heroStories[currentSlide].icon, {
              ...
              <div className="space-y-4">
                <h1 key={currentSlide} className="text-5xl lg:text-7xl font-poppins font-black leading-tight animate-fade-in">
                  {heroStories[currentSlide].title}
                </h1>
                <p key={`subtitle-${currentSlide}`} className="text-xl text-muted-foreground leading-relaxed max-w-2xl animate-fade-in" style={{
                animationDelay: '0.1s'
              }}>
            ...
            <div className="animate-fade-in-right px-[27px] my-[2px] py-px mx-[120px]">
              <Card className="p-6 bg-white/30 backdrop-blur-sm border border-white/20 shadow-2xl max-w-md mx-auto">
                <CardContent className="p-0">
                  {/* Header */}
      ...
      <section className="py-24 bg-gradient-to-b from-neutral-50 to-primary-50/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            return <div key={index} className="relative h-80 rounded-2xl overflow-hidden group cursor-pointer animate-scale-in hover:scale-105 transition-all duration-300" style={{
      ...
      <section className="py-24 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative h-[600px] overflow-hidden rounded-3xl">
            ...
            {processSteps.map((step, index) => <div key={index} className={`absolute inset-0 transition-all duration-1000 ease-in-out ${index === currentProcessStep ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}>
            ...
            <div className="relative z-10 h-full flex items-center">
              <div className="max-w-2xl mx-auto px-8 text-center text-white">
                {/* Step Number */}
                <div key={`step-${currentProcessStep}`} className="inline-flex items-center justify-center w-20 h-20 bg-primary text-primary-foreground rounded-full font-bold text-2xl mb-8 animate-scale-in">
                  {processSteps[currentProcessStep].step}
                </div>

                {/* Title */}
                <h3 key={`title-${currentProcessStep}`} className="text-4xl lg:text-5xl font-poppins font-bold mb-6 animate-fade-in">
                  {processSteps[currentProcessStep].title}
                </h3>

                {/* Description */}
                <p key={`desc-${currentProcessStep}`} className="text-xl leading-relaxed mb-8 animate-fade-in" style={{
                animationDelay: '0.2s'
              }}>
                ...
                {/* Icon */}
                <div key={`icon-${currentProcessStep}`} className="flex justify-center animate-scale-in" style={{
                animationDelay: '0.4s'
              }}>
            ...
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20">
              <div className="flex items-center gap-3">
                {processSteps.map((_, index) => <button key={index} onClick={() => setCurrentProcessStep(index)} className={`h-3 w-8 rounded-full transition-all duration-300 ${index === currentProcessStep ? 'bg-white' : 'bg-white/30 hover:bg-white/50'}`} />)}
              </div>
            </div>

            {/* Navigation Arrows */}
            <button onClick={() => setCurrentProcessStep(prev => prev === 0 ? processSteps.length - 1 : prev - 1)} className="absolute left-8 top-1/2 transform -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-sm transition-all duration-200">
              <ArrowRight className="h-6 w-6 text-white rotate-180" />
            </button>
            
            <button onClick={() => setCurrentProcessStep(prev => (prev + 1) % processSteps.length)} className="absolute right-8 top-1/2 transform -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 rounded-full backdrop-blur-sm transition-all duration-200">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            {/* Quality Beef */}
            <div className="group animate-scale-in hover:scale-105 transition-all duration-300" style={{
            animationDelay: '0.1s'
          }}>
              <Card className="card-elevated overflow-hidden">
                <CardContent className="p-0">
